...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    for dev in DEVICES:
        if dev["type"] == "switch":
            entities.append(ISmartModbusSwitch(coordinator, dev["name"], dev["device_id"], dev["input"], dev["output"], modbus))
        elif dev["type"] == "toggle_switch":
            entities.append(ISmartModbusToggleSwitch(coordinator, dev["name"], dev["device_id"], dev["input"], dev["output"], modbus))
    async_add_entities(entities)

class ISmartModbusToggleSwitch(ISmartModbusBitEntity, SwitchEntity):
...
class ISmartModbusSwitch(ISmartModbusBitEntity, SwitchEntity):
    ...
    async def async_turn_off(self, **kwargs):
        await self._write_coil(self._coil, 0)
